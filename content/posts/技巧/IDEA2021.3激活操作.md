+++
author = "cvooc"
title = "IDEA2021.3激活操作"
date = "2023-02-18 14:28:08"
description = "IDEA2021.3激活操作"
tags = [
    "技巧",
    "idea",
]
+++

# IDEA2021.3+激活操作

> 截至 2023 年 2 月新版教程已更新, 现在有更简单的方法了, 请参考 **使用说明(新)**

由于 IDEA 机制更新[Z 大](https://zhile.io/)的重置试用时间插件**IDE Eval Resetter**在 2021.3 版本中彻底失效了，于是 Z 大更新了适用于新版 IDEA 的开源库：[~~ja-netfilter(github)~~](https://github.com/ja-netfilter/ja-netfilter)、[ja-netfilter(gitee)](https://gitee.com/ja-netfilter/ja-netfilter)

## 使用说明(新)

### 下载补丁

请直接访问[key 下载地址](https://jetbra.in/s), 并下载插件

1. 随便挑一个能访问的页面进入

![跳转页](/static/img/IDEA2021.3激活操作/2.png)

2. 下载 jetbra.zip 插件包

![下载插件](/static/img/IDEA2021.3激活操作/3.png)

3. 后续可参考 激活操作 2~3 步即可完成激活

## 使用说明

### ~~下载补丁~~

前往 releases 页面下载最新版的补丁文件,需要注意的是**mymap**插件的仓库链接不是**原作者**,具体来源我没有找到,后[**另外的作者**](https://www.zfkun.com/ja-netfilter-mymapy-plugin.html)将其反编译后发布至了 github

-   [ja-netfilter](https://gitee.com/ja-netfilter/ja-netfilter/releases)
-   [ja-netfilter-mymap-plugin](https://github.com/zfkun/ja-netfilter-mymap-plugin/releases)

### ~~补丁目录~~

```javascript

ja-netfilter
├── plugins
│   ├── mymap-v1.0.1.jar    // mymap插件放置在这里
│   ├── url.jar             // ja-netfilter插件自带文件
│   ├── hideme.jar          // ja-netfilter插件自带文件
│   └── dns.jar             // ja-netfilter插件自带文件
├── ja-netfilter.jar        // ja-netfilter插件自带文件
└── janf_config.txt         // ja-netfilter配置文件,若不存在则自行创建一个txt文件即可

```

### 激活操作

1. ~~编写**janf_config.txt**配置文件~~

```yaml

# ja-netfilter配置项,此处不用改动,具体参数说明可以参考插件的readme文件
[DNS]
EQUAL,jetbrains.com

[URL]
PREFIX,https://account.jetbrains.com/lservice/rpc/validateKey.action

# mymap插件配置项
[MyMap]

# 被许可人姓名，也就是Licensed to xxx里的xxx信息；
EQUAL,licenseeName->retrocode

# 宽限期；
EQUAL,gracePeriodDays->30

# 激活的时长，也就是熟悉的2099-12-31的配置；
EQUAL,paidUpTo->5000-12-31

```

2. 在 IDEA 安装目录下的 bin 文件夹中找到**idea64.exe.vmoptions**,在最后一行追加插件配置路径

```yaml
-javaagent:c:/ja-netfilter/ja-netfilter.jar=jetbrains
```

3. 启动 IDEA 输入激活码并激活即可,激活码若失效可以从[热心大佬分享的 key](https://jetbra.in/s)获得最新的激活码.

示例 key:

```

IZ6BB8BE9L---

```

![激活IDEA](/static/img/IDEA2021.3激活操作/1.png)

4. 至此 IDEA 已经激活完毕,后续版本更新后若失效重新激活一次即可.
